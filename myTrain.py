...
# 记录最佳joint acc，earlyStop计数器，当前在dev上的accuracy
avg_best, cnt, acc = 0.0, 0, 0.0

# 数据预处理，加载数据，训练：training=True
...
for epoch in range(200):
    ...
    pbar = tqdm(enumerate(train),total=len(train))   # 进度条
    for i, data in pbar:
        model.train_batch(data, int(args['clip']), SLOTS_LIST[1], reset=(i==0))
        model.optimize(args['clip'], i)
        pbar.set_description(model.print_loss())
        # print(data)
        # exit(1)

    if((epoch+1) % int(args['evalp']) == 0):  # 参数evaluation period 'evalp'默认为1，即每个epoch评估一次
